    ...
    class RTLink(RTComponent):
        ...
        def __calculateGeometry__(self, for_entities=None):
            ...
            for i in range(len(self.relationships)):
                ...
                _operations_.append(pl.col(_fm_).replace(self.pos, default=None, return_dtype=pl.List(pl.Float32)).list.get(0).alias(_fmx_))
                _operations_.append(pl.col(_fm_).replace(self.pos, default=None, return_dtype=pl.List(pl.Float32)).list.get(1).alias(_fmy_))
                self.xcols.append(_fmx_), self.ycols.append(_fmy_)
                _to_  = _rel_[1]
                _tox_ = f'__rel{i}_to_wx__'
                _toy_ = f'__rel{i}_to_wy__'
                _operations_.append(pl.col(_to_).replace(self.pos, default=None, return_dtype=pl.List(pl.Float32)).list.get(0).alias(_tox_))
                _operations_.append(pl.col(_to_).replace(self.pos, default=None, return_dtype=pl.List(pl.Float32)).list.get(1).alias(_toy_))

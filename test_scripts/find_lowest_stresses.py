...
for i in range(len(_patterns_)):
    ...
    _distances_ = rt.distanceDictionary(_g_)
    _stress_min_, _algo_min_, _algo_min_str_ = None, None, None
    for k in range(3):
        for _trial_ in range(3):
            for _algo_no_ in range(2):
                t0 = time.time()
                if _algo_no_ == 0: _algo_, _algo_str_  = rtsvg.PolarsForceDirectedLayout(_g_, k=k, distances=_distances_), 'PFDL'
                else:              _algo_, _algo_str_  = ConveyProximityLayout          (_g_, k=k, distances=_distances_), 'CPL'
                t1 = time.time()
